...
CdcCellGeometryParameter::CdcCellGeometryParameter(G4String name, G4String opt )
	: SimpleGeometryParameter(name, "S") // Use a not-empty opt to stop inherited class from creating new GeometryParameterMessenger
{
	if ( opt == "" ){
		CdcCellGeometryParameterMessenger *pointer = new CdcCellGeometryParameterMessenger(this, name);
		//std::cout<<"======>In CdcCellGeometryParameter, new CdcCellGeometryParameterMessenger at ("<<(void*)pointer<<")!"<<std::endl;
		set_GeometryParameterMessenger(pointer);
	}
}

CdcCellGeometryParameter::~CdcCellGeometryParameter(){
	printf("~CdcCellGeometryParameter");
}

//---------------------------InitFromFile---------------------------------
//will be called in CdcCellGeometrySvc::SetCdc
void CdcCellGeometryParameter::InitFromFile( G4String file_name ){
	...
	if(!fin_card){
		DEBUG("CdcCellGeometryParameter::InitFromFile(), cannot find subgeometry card: \""+file_name+"\"");
	}
	std::string s_card;
	while(getline(fin_card,s_card)){
		...
		if( status > 0 ){
			G4String content = "CdcCellGeometryParameter: unknown line:" + s_card + "\n  in file " + file_name + "! Will ignore this line!";
			DEBUG(content,0);
		}
	}
	fin_card.close();
	if (CheckInfo()){
		DEBUG("CdcCellGeometryParameter::InitFromFile(), insufficient simple solid info");
	}
	Calculate();
}

void CdcCellGeometryParameter::Dump(){
	DumpInfo();
}

//-----------------------------------Special functions---------------------------------------------

bool CdcCellGeometryParameter::CheckInfo(){
...
void CdcCellGeometryParameter::Preset(){
...
int CdcCellGeometryParameter::GetValue(G4String s_card){
...
//--------------------------Calculate---------------------------------
//
void CdcCellGeometryParameter::Calculate(){
...
//---------------------------DumpInfo---------------------------------
//output information
void CdcCellGeometryParameter::DumpInfo() {
	SimpleGeometryParameter::DumpInfo();// dump info of its Parent Class first
	std::cout<<"------CdcCellGeometry info :--------"<<std::endl;
