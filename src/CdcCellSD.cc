...
typedef HepGeom::Vector3D<double> HepVector3D;
	...
	CdcCellSD::CdcCellSD(G4String name, MyVGeometryParameter* pointer)
:MySD(name, pointer), hitsCollection(0)
{
	...
	if (!m_GeometryParameter){
		//		std::cout<<"In CdcCellSD::CdcCellSD, cannot get CdcCellGeometryParameter pointer!!!"<<std::endl;
		G4Exception("CdcCellSD::CdcCellSD()",
...
void CdcCellSD::Initialize(G4HCofThisEvent* HCE)
{
	hitsCollection = new CdcHitsCollection
		(SensitiveDetectorName,collectionName[0]);
	std::cout<<"====>In CdcCellSD::Initialize(), created hitsCollection at ("<<(void*)hitsCollection<<std::endl;
	G4int HCID = -1;
	HCID = G4SDManager::GetSDMpointer()->GetCollectionID(collectionName[0]);
	if ( HCID < 0 ){
		G4Exception("CdcCellSD::Initialize()",
				"InvalidSetup", FatalException,
...
void CdcCellSD::ReadOutputCard(G4String filename){
	ReSet();
	G4String volName = get_VolName();
	std::ifstream fin_card(filename);
	if(!fin_card){
		std::cout<<"In CdcCellSD::ReadOutputCard, cannot open "<<filename<<"!!!"<<std::endl;
		G4Exception("CdcCellSD::ReadOutputCard()",
				"InvalidSetup", FatalException,
	...
	while(getline(fin_card,s_card)){
		...
		if ( n_output_section_symbol == 0 ){
			...
			if ( name == BRANCHSECTIONNAME ){
				...
				std::cout<<"In CdcCellSD::ReadOutputCard, unknown name: "<<name<<" in file "<<filename<<std::endl;
	...
	if ( n_output_section_symbol <= 1 ){
		...
		std::cout<<"In CdcCellSD::ReadOutputCard, failed to find enough section seperators for output in file "<<filename<<std::endl;
	...
	if ( n_filter_section_symbol<= 1 ){
		std::cout<<"*****************WARNING********************"<<std::endl;
		std::cout<<"In CdcCellSD::ReadOutputCard, failed to find enough section seperators for filter in file "<<filename<<std::endl;
...
//-----------------------------------ProcessHits----------------------------------------------
//Will be called by geant4 automatically everytime a step in CdcCellSD generated
G4bool CdcCellSD::ProcessHits(G4Step* aStep,G4TouchableHistory* touchableHistory)
{

	std::cout<<"In CdcCellSD::ProcessHits()"<<std::endl;
	...
	if(status){
		std::cout<<"In CdcCellSD::ProcessHits(), unknown ReplicaNo = "<<ReplicaNo<<"!!! Will not generate hit"<<std::endl;
		return false;
	}
	...
	if (status){
		std::cout<<"In CdcCellSD::ProcessHits(), cannot find closest point!!! will not generate hit!"<<std::endl;
		return false;
	}
...
void CdcCellSD::EndOfEvent(G4HCofThisEvent*){
	if (verboseLevel>0) {
...
//-----------------------------------FindClosestPoint----------------------------------------------
//private
int CdcCellSD::FindClosestPoint(G4ThreeVector &closestPoint_pos, double &driftD,
	...
	if ( B_amplitude == 0 || pT == 0 ){
		...
		if (mom_direction.mag()==0){
			std::cout<<"WARNING!!! In CdcCellSD::FindClosestPoint(), momentum is zero!"<<std::endl;
			return 1;
		}
	...
	//***********************3, r = !0 && B_direction does not parallel wire******************************
		...
		if ( (type_left == type_right && type_right <= 1 && root_left != root_right) || (type_left == 1 && type_right == 0) ){
			...
			while (1){
				...
				if ( iTry++ >= nTry_max ){
					//std::cout<<"WARNING: in CdcCellSD::FindClosestPoint(), iTry reached maximum limit "<<iTry<<std::endl;
					break;
				}
			}
...
int CdcCellSD::check_type(G4double a[6], G4double phi){
	//0: f'>=0 && f''<0;
	//1: f'<0 && f''<=0;
	//2: f'<0 && f''>=0;
	//3: f'>=0 && f''>0;
...
double CdcCellSD::NewtonRootFinding(G4double a[6], G4double phi, G4int nTry_max, G4double torl, G4double &error){
	G4double delta = 10e10;
	G4int iTry = 0;
	while( fabs(delta) > torl ){
		iTry++;
		if( iTry > nTry_max ){
			//std::cout<<"WARNING: in CdcCellSD::NewtonRootFinding(), iTry reached maximum limit "<<iTry<<std::endl;
			break;
		}
		//d(D^2/r^2)/d(phi)) = a[0] + a[1]*sin(2*phi) + a[2]*sin(phi) + a[3]*cos(phi) + a[4]*phi*cos(phi) + a[5]*phi
		//dd(D^2/r^2)/dd(phi)) = 2*a[1]*cos(2*phi) + (a[2]+a[4])*cos(phi) - a[3]*sin(phi) - a[4]*phi*sin(phi) + a[5]
		...
		if ( k == 0 ){
			//std::cout<<"WARNING: in CdcCellSD::NewtonRootFinding(), k = 0!!!"<<std::endl;
			phi += pi/50;
		}
		else phi -= delta/k;
	}
...
double CdcCellSD::calculateDist2(G4double b[4], G4double phi){
	//b[0] = theta_helix;
	//b[1] = k;
	//b[2] = A;
	//b[3] = B;
...
int CdcCellSD::findSmallest( double a, double b, double c){
	if ( a < b ){
		if ( a < c ) return 0;
		else return 2;
	}
