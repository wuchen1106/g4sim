...
CdcCellGeometrySvc::CdcCellGeometrySvc(G4String name, G4String opt )
	: SimpleGeometrySvc(name, "S") // Use a not-empty opt to stop inherited class from creating new GeometryParameter
{
	if ( opt == "" ){//called directly by user. set up parameter class.
		CdcCellGeometryParameter *pointer = new CdcCellGeometryParameter(name);
		//		std::cout<<"======>In CdcCellGeometrySvc, new CdcCellGeometryParameter at ("<<(void*)pointer<<")!"<<std::endl;
		set_GeometryParameter(pointer);
	}
}

CdcCellGeometrySvc::~CdcCellGeometrySvc(){
	printf("~CdcCellGeometrySvc");
}

//------------------------Modify-------------------------------
void CdcCellGeometrySvc::set_GeometryParameter( CdcCellGeometryParameter* val ){
...
//---------------------------SetGeometry---------------------------------
//
G4VPhysicalVolume* CdcCellGeometrySvc::SetGeometry(){
...
//-------------------------Special functions-----------------------------
//=> ConstructVolumes
void CdcCellGeometrySvc::ConstructVolumes(){
	...
	if (!SignalWireMat || !FieldWireMat || !CellMat ){
		std::cout<<"In CdcCellGeometrySvc, Material is not defined!"<<std::endl;
		G4Exception("CdcCellGeometrySvc::SetGeometry()",
	...
	for( int layerId = 0; layerId < layerNo; layerId++ ){
		...
		G4VSensitiveDetector* aSD = MyDetectorManager::GetMyDetectorManager()->GetSD(SDVolumeName, SD_name, const_cast<CdcCellGeometrySvc*>(this) );
...
//=>Place Volumes
G4VPhysicalVolume* CdcCellGeometrySvc::PlaceVolumes(){
